...
def _read_metadata(file_path):
    ...
    '''
    metadata = pd.read_csv(file_path)
    metadata = metadata[["id","title","genres","original_language","overview","release_date","vote_average", "vote_count"]]
    ...
    metadata['genres'] = metadata.genres.apply(lambda x : unpacking_genre(x) )
    metadata["release_date"] = metadata.release_date.apply(lambda x : transform_date(x) )  
    return metadata
...
def _read_keywords(file_path):
    ...
    keywords['keywords'] = keywords.keywords.apply(lambda x : unpacking_keywords(x) )
    return keywords
